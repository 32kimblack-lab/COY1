...
final class APIClient {
	...
	func updateCollection(
		...
		if let allowedUsers = allowedUsers {
			...
			print("🔧 APIClient.updateCollection: Including allowedUsers=\(allowedUsers.count) users")
		}
		
		if let deniedUsers = deniedUsers {
			body["deniedUsers"] = deniedUsers  // Send even if empty array
			print("🔧 APIClient.updateCollection: Including deniedUsers=\(deniedUsers.count) users")
		}
		
		print("🔧 APIClient.updateCollection: Full request body: \(body)")
		
		// CRITICAL: Allow body with only access arrays (allowedUsers/deniedUsers)
		// These are valid updates even if other fields are nil
		if body.isEmpty {
	...
	func updateUser(
		...
		if let starredIds = starredPostIds, let add = addStarred {
			if add {
				body["addStarredPostIds"] = starredIds
			} else {
				body["removeStarredPostIds"] = starredIds
			}
		}
	...
	/// Hide a post
	func hidePost(postId: String, userId: String) async throws -> UserResponse {
		var request = try await createRequest(endpoint: "/users/\(userId)/hide-post", method: "POST")
	...
	/// Unhide a post
	func unhidePost(postId: String, userId: String) async throws -> UserResponse {
		var request = try await createRequest(endpoint: "/users/\(userId)/unhide-post", method: "POST")
		let body: [String: Any] = ["postId": postId]
		request.httpBody = try JSONSerialization.data(withJSONObject: body)
		request.setValue("application/json", forHTTPHeaderField: "Content-Type")
	...
	/// Hide a collection
	func hideCollection(collectionId: String, userId: String) async throws -> UserResponse {
		var request = try await createRequest(endpoint: "/users/\(userId)/hide-collection", method: "POST")
		let body: [String: Any] = ["collectionId": collectionId]
		request.httpBody = try JSONSerialization.data(withJSONObject: body)
		request.setValue("application/json", forHTTPHeaderField: "Content-Type")
	...
	/// Unhide a collection
	func unhideCollection(collectionId: String, userId: String) async throws -> UserResponse {
		var request = try await createRequest(endpoint: "/users/\(userId)/unhide-collection", method: "POST")
		let body: [String: Any] = ["collectionId": collectionId]
		request.httpBody = try JSONSerialization.data(withJSONObject: body)
		request.setValue("application/json", forHTTPHeaderField: "Content-Type")
	...
	/// Report a post
	func reportPost(postId: String, userId: String, category: String, additionalDetails: String?) async throws {
		var request = try await createRequest(endpoint: "/posts/\(postId)/report", method: "POST")
		var body: [String: Any] = [
			"userId": userId,
			"category": category
		]
		if let details = additionalDetails, !details.isEmpty {
			body["additionalDetails"] = details
		}
		request.httpBody = try JSONSerialization.data(withJSONObject: body)
		request.setValue("application/json", forHTTPHeaderField: "Content-Type")
	...
	/// Get deleted collections for current user
	func getDeletedCollections() async throws -> [CollectionResponse] {
		print("📋 APIClient: Fetching deleted collections")
		let request = try await createRequest(endpoint: "/collections/deleted", method: "GET")
		let (data, response) = try await URLSession.shared.data(for: request)
		try validateResponse(response, data: data)
		let collections = try JSONDecoder().decode([CollectionResponse].self, from: data)
		print("✅ APIClient: Found \(collections.count) deleted collections")
		return collections
	}
}
...
struct CollectionResponse: Codable {
	...
	let deletedAt: String? // ISO8601 date string for deleted collections
}
